...
@pytest.mark.asyncio
@patch("app.api.books.service")
async def test_get_books(mock_service, client):
    ...
    mock_service.get_books.assert_awaited_once()

@pytest.mark.asyncio
@patch("app.api.books.service")
async def test_get_book_success(mock_service, client):
    book_id = uuid4()
...
@pytest.mark.asyncio
@patch("app.api.books.service")
async def test_get_book_not_found(mock_service, client):
    book_id = uuid4()
...
@pytest.mark.asyncio
@patch("app.api.books.service")
async def test_create_book(mock_service, client):
    book_data = {
        "title": "New Book",
        "author": "Author",
        "description": "Some description",
...
@pytest.mark.asyncio
@patch("app.api.books.service")
async def test_delete_book(mock_service, client):
    book_id = uuid4()
